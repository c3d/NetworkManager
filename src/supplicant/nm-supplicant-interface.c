                ...
                const char *reason)
{
	...
	_nm_supplicant_manager_remove_interface (priv->supplicant_manager,
	                                         self,
	                                         TRUE,
	                                         "cannot get supplicant state");
}
...
set_state (NMSupplicantInterface *self, NMSupplicantInterfaceState new_state)
{
	...
	_emit_signal_state (self, priv->state, old_state, priv->disconnect_reason);
}
...
_starting_check_ready (NMSupplicantInterface *self)
{
	...
	if (!NM_SUPPLICANT_INTERFACE_STATE_IS_OPERATIONAL (priv->supp_state)) {
		...
		set_state_down (self, "failure to get supplicant state");
		return;
	}
                          ...
                          GVariant *properties)
{
	...
	gint32 v_i32;
	GVariant *v_v;
	gboolean log_driver_info = FALSE;
...
assoc_set_ap_scan_cb (GVariant *ret, GError *error, gpointer user_data)
{
	...
	_LOGT ("assoc["NM_HASH_OBFUSCATE_PTR_FMT"]: set interface ap_scan to %d",
	       NM_HASH_OBFUSCATE_PTR (priv->assoc_data),
...
_nm_supplicant_interface_set_state_down (NMSupplicantInterface * self)
{
	...
	set_state_down (self, "interface unregistered");
}
