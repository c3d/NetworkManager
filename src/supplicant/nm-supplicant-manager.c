                             ...
                             const char *reason)
{
	...
	name_owner = nm_supplicant_interface_get_name_owner (supp_iface);
	iface_path = nm_supplicant_interface_get_object_path (supp_iface);

	was_tracked = !c_list_is_empty (&supp_iface->supp_lst);
	...
	if (was_tracked) {
		...
		force_remove_from_supplicant |= supp_iface->supp_iface_created_by_us;
	}

	_LOGD ("remove%s interface \"%s\" on %s (%s)%s",
	       was_tracked ? "" : " (not tracked)",
	       iface_path->str,
	       name_owner->str,
	       reason,
...
void
_nm_supplicant_manager_remove_interface (NMSupplicantManager *self,
                                         NMSupplicantInterface *supp_iface,
                                         gboolean remove_from_supplicant,
                                         const char *reason)
{
	g_return_if_fail (NM_IS_SUPPLICANT_MANAGER (self));
	g_return_if_fail (NM_IS_SUPPLICANT_INTERFACE (supp_iface));
	g_return_if_fail (reason);

	_supp_iface_remove_complete (self,
	                             supp_iface,
	                             remove_from_supplicant,
	                             TRUE,
	                             reason);
}
...
{
	_supp_iface_remove_complete (data, NM_SUPPLICANT_INTERFACE (where_the_object_was), FALSE, FALSE, "object unreference");
}
                        ...
                        const char *reason)
{
	...
	g_object_weak_unref (G_OBJECT (supp_iface),
	                     _supp_iface_weak_cb,
	                     self);
	_supp_iface_remove_complete (self, supp_iface, FALSE, TRUE, reason);
}
